...
image_name="coilyco-flight-deck/node-stats-mcp"
...
case "${sha}" in
  *[!0-9a-f]*|"")
    echo "node-stats-mcp source sha is not a lowercase hexadecimal commit id." >&2
    exit 1
    ;;
esac
if [ "${#sha}" -ne 40 ]; then
  echo "node-stats-mcp source sha must be a full 40-character commit id." >&2
  exit 1
fi
